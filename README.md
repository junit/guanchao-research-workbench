...
| [docs/IMPLEMENTATION.md](docs/IMPLEMENTATION.md) | 实现交接（从零重建工作台） |
...
1. 先做**实时大事件扫描**；地缘是动态发现，不是固定地区打卡表。可并行轻扫策展 X 观察表（发现候选 only）。
...
- 策展 **X 发现层**：`market/sources/x-watchlist.md` + 清单/Schema/技能对齐；禁个人关注列表；升格大事件须通讯社/一手交叉。
...
- 市场空骨架已完整补齐：reviews 模板、comps、dashboard 空壳与 `calendar/world-tracks/INDEX.md` 均已提供，构成完整空骨架，便于导入后填充。
