...
8. Cross-asset side checks (gold + crypto): crypto trades 24h — do **not** add a separate always-on cron. Every A/HK and US session brief gets a light refresh of `dashboards/crypto.md` (BTC/ETH spot + ~24h, and ~7d when available); this is not US-only and must not use “reuse if <6h” as the normal rule. An as-of >6h reading is stale fallback only when refresh fails, and must be labeled stale rather than presented as the current session refresh. **Full** A/HK and US briefs must include a BTC/ETH row; US full briefs also include gold. Every **short** brief includes one crypto line; promote it to 大事件 only if |24h|≥5%, vs prior brief jump ≥3%, hard regulatory/ETF/exchange news, or a clear co-move/divergence with the market that needs elevation. A-share holiday gate does **not** skip crypto.
